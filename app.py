...
def get_objetos():
    try:
            ...
            codigos_centros = request.args.getlist('codigos_centros')
            codigos_canchas = request.args.getlist('codigos_canchas')
    
            # Validar que los parámetros requeridos estén presentes
            if not fecha or not codigos_centros or not codigos_canchas:
                return jsonify({"error": "Parámetros requeridos: 'fecha', 'codigos_centros' y 'codigos_canchas'"}), 400
    
            # Convertir listas de códigos en cadenas separadas por comas
